...
// ─── WhatsApp messages par service ────────────────────────────────────────────

const WA_MESSAGES: Record<string, string> = {
  ecrans:     "Bonjour, je viens du site Com'9. Je souhaite prendre rendez-vous pour un changement d'écran.",
  batteries:  "Bonjour, je viens du site Com'9. Je souhaite prendre rendez-vous pour un changement de batterie.",
  diagnostic: "Bonjour, je viens du site Com'9. Je souhaite prendre rendez-vous pour un diagnostic.",
}
    ...
    <div className="rounded-xl overflow-hidden transition-all duration-300"
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            {/* Column headers */}
            <div className="grid grid-cols-3 px-4 py-2 font-mono text-[9px] tracking-[0.18em] uppercase"
            ...
            <div className="divide-y" style={{ borderColor: 'rgba(0,209,255,0.04)' }}>
              {serie.rows.map((row, idx) => (
                <div key={row.model}
                  className="grid grid-cols-3 items-center px-4 py-3 transition-colors duration-150"
                ...
                >
                  <span className="font-space text-xs pr-2" style={{ color: 'rgba(255,255,255,0.96)' }}>
                    {row.model}
                  </span>
  ...
  return (
    <div className="space-y-2.5">
      ...
      <div className="pt-6 mt-2" style={{ borderTop: '1px solid rgba(0,209,255,0.08)' }}>
        <WaButton message={WA_MESSAGES[cat.id]} label="Prendre rendez-vous sur WhatsApp" />
      </div>
    </div>
  )
}
    ...
    >
      <div className="glass-card rounded-2xl overflow-hidden">
        ...
        <div className="px-7 pb-7">
          <WaButton message={WA_MESSAGES.diagnostic} label="Réserver sur WhatsApp" />
        </div>
      </div>
    </motion.div>
  )
      ...
      <div className="max-w-3xl mx-auto px-5 md:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-8"
            style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(0,209,255,0.1)' }}>
            <span className="w-1 h-1 rounded-full shrink-0" style={{ background: 'rgba(0,209,255,0.55)' }} />
            <p className="font-mono text-[9px] tracking-[0.2em] uppercase whitespace-nowrap"
