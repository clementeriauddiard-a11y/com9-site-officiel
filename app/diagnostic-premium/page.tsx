...
// Mot de passe géré dans lib/config.ts
// ─────────────────────────────────────────────────────────────────────────────
...
function AccessGate({ onUnlock }: { onUnlock: () => void }) {
  ...
  const [phase,    setPhase]    = useState(0) // reveal animation
  ...
  useEffect(() => {
    ...
    const t4 = setTimeout(() => { setPhase(4); inputRef.current?.focus() }, 1800)
      ...
      >
        <div className="relative rounded-2xl overflow-hidden"
          style={{
            ...
            boxShadow: error
              ? '0 0 60px rgba(248,113,113,0.08), inset 0 1px 0 rgba(248,113,113,0.08)'
          ...
          <div className="relative z-10 px-8 py-10">
            ...
            >
              ...
              <div className="absolute inset-1.5 rounded-full flex items-center justify-center"
                ...
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" className="w-7 h-7"
                  style={{ color: error ? '#f87171' : '#00d1ff', transition: 'color 0.3s' }}>
              ...
              <motion.div
                ...
                style={{ boxShadow: error ? '0 0 24px rgba(248,113,113,0.25)' : '0 0 24px rgba(0,209,255,0.2)', transition: 'box-shadow 0.3s' }}
              />
            </motion.div>
            ...
            >
              Interface réservée à l&apos;usage interne Com&apos;9.<br />
            ...
            >
              ...
              <div className="relative">
                <div className="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none">
                  <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-3.5 h-3.5"
                    style={{ color: error ? 'rgba(248,113,113,0.6)' : 'rgba(0,209,255,0.45)', transition: 'color 0.3s' }}>
                ...
                <input
                  ...
                  onFocus={e => {
                    if (!error) {
                      ...
                      e.currentTarget.style.boxShadow = '0 0 20px rgba(0,209,255,0.08)'
                    }
                  }}
                  onBlur={e => {
                    if (!error) {
              ...
              <motion.button
                ...
                onMouseEnter={e => {
                  if (value) {
                    e.currentTarget.style.background = 'rgba(0,209,255,0.16)'
                    e.currentTarget.style.borderColor = 'rgba(0,209,255,0.6)'
                    e.currentTarget.style.boxShadow = '0 0 32px rgba(0,209,255,0.15)'
                  }
                }}
                onMouseLeave={e => {
                  if (value) {
                    ...
                    e.currentTarget.style.boxShadow = '0 0 24px rgba(0,102,255,0.1)'
                  }
                }}
              >
                <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-3.5 h-3.5">
            ...
            >
              <p className="font-mono text-[8px] tracking-[0.18em] uppercase"
                style={{ color: 'rgba(234,251,255,0.15)' }}>
                Com&apos;9 · Système de diagnostic officiel
              </p>
            </motion.div>
...
export default function DiagnosticPremiumPage() {
  const [unlocked, setUnlocked] = useState(false)
  const [closing,  setClosing]  = useState(false)

  function handleClose() {
    setClosing(true)
    // Small delay to let DiagnosticModal exit animation play
    setTimeout(() => { setUnlocked(false); setClosing(false) }, 300)
  }
  ...
  return (
    <>
      ...
      {/* Premium diagnostic — full screen */}
      <AnimatePresence>
        {unlocked && !closing && (
          <DiagnosticModal onClose={handleClose} />
        )}
      </AnimatePresence>
    </>
  )
